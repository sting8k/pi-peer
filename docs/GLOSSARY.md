...
In pi-peer, the user-facing id of a session: `peer-<last 3 chars of the full
session id>` (e.g. `peer-123`). It is what `talk_sessions` returns and what
`talk_latest`/`talk_to` examples lead with. The full session id remains the
internal identity for artifact paths, routes, inbox/reply addressing, and
history correlation; the public id is a presentation-only alias produced by
one central formatter, and two live records with the same public id fail
closed as ambiguous. Target resolution accepts the public id or a unique
display name; raw full session ids and id prefixes are not targets.
