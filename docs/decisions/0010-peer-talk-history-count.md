...
- On `session_start`/resume and on `agent_settled` a peer rebuilds its history from its own current lineage (completed entries only). Session entries are persisted before the settlement boundary, so rebuilding after retries, compaction, and queued continuations yields stable entry-based ids and no duplicate risk; an empty current lineage clears any stale history.
