...
| `story_id` | TEXT | Standard+ when work maps to one story | Story id from the `story` table. Use the main story when one trace covers several; list the rest in `notes`. | `US-009` |
...
| `notes` | TEXT | Optional | Free text for review context that does not fit other fields. | `Trace covers US-009 and US-010.` |
